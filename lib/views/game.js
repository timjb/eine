(function() {
  ...
  App.Views.Game = (function() {
    ...
    Game.prototype.initialize = function(options, humanPlayer) {
      ...
      return this.model.players.bind('all', this.render);
    };
    Game.prototype.render = function() {
      var closedCard, humanPlayerView, nextPlayers, openCard, positions;
      $(this.el).html('');
      openCard = $((new Card({
        model: this.model.get('open')
      })).render().el).addClass('open').appendTo(this.el);
      ...
      closedCard.appendTo(this.el);
      humanPlayerView = new Player({
        model: this.humanPlayer
      }, true);
      $(humanPlayerView.render().el).addClass('bottom').appendTo(this.el);
